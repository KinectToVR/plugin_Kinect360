...
{
    public static readonly Guid AudioEndpoint = new("{c166523c-fe0c-4a94-a586-f1a80cfbbf3e}");
    public static readonly Guid Bluetooth = new("{e0cbf06c-cd8b-4647-bb8a-263b43f0f974}");
    public static readonly Guid BTW = new("{95c7a0a0-3094-11d7-a202-00508b9d7d5a}");
    ...
    public static readonly Guid Keyboards = new("{4d36e96b-e325-11ce-bfc1-08002be10318}");
    public static readonly Guid KinectForWindows = new("{3a0339cd-b5f0-421c-8661-f243eef1528c}");
    public static readonly Guid Mouse = new("{4d36e96f-e325-11ce-bfc1-08002be10318}");
    public static readonly Guid USB = new("{36fc9e60-c465-11cf-8056-444553540000}");
    public static readonly Guid Media = new("{4d36e96c-e325-11ce-bfc1-08002be10318}");
    public static readonly Guid USBDevice = new("{88bae032-5a81-49f0-bc3d-a4ff138216d6}");
    public static readonly Guid Unknown = new("{00000000-0000-0000-0000-000000000000}");
}
...
public class DeviceTree : IDisposable
{
    ...
    private readonly IDependencyInstaller.ILocalizationHost _host;

    // flat collection of all devices found
    private List<DeviceNode> _deviceNodes;

    public DeviceNode RootNode => _rootNode;

    public List<DeviceNode> DeviceNodes => _deviceNodes;

    public DeviceTree(IDependencyInstaller.ILocalizationHost host)
    {
        _host = host;
        EnumerateDevices();
    }
    ...
    private void EnumerateDevices()
    {
        ...
        if (CM_Connect_Machine(null, ref _machineHandle) != 0) return;
        ...
        try
        {
            ...
            _rootNode = new DeviceNode(_rootDeviceHandle, null, _machineHandle, _host);
        }
        finally
        {
            DisconnectFromMachine();
    ...
    private static extern uint CM_Reenumerate_DevNode_Ex(ref IntPtr deviceHandle, uint flags, IntPtr machineHandle);
...
public class DeviceNode : IDisposable
{
    ...
    private IDependencyInstaller.ILocalizationHost _host;

    public DeviceNode ParentDevice => _parentDevice;
    ...
    public List<DeviceNode> Children => _children;

    public Guid ClassGuid
    {
        get
        {
            var buffer = GetProperty(DevRegProperty.ClassGuid);
            var guid = new Guid();
    ...
    public string Description => GetProperty(DevRegProperty.DeviceDescription);

    public string FriendlyName => GetProperty(DevRegProperty.FriendlyName);

    public string EnumeratorName => GetProperty(DevRegProperty.EnumeratorName);

    public string LocationInfo => GetProperty(DevRegProperty.LocationInfo);

    public DeviceNode(IntPtr deviceHandle, DeviceNode parentDevice, IDependencyInstaller.ILocalizationHost host) :
        this(deviceHandle, parentDevice, parentDevice._machineHandle, host)
    {
    }

    public DeviceNode(IntPtr deviceHandle, DeviceNode parentDevice, IntPtr machineHandle, IDependencyInstaller.ILocalizationHost host)
    {
        _host = host;
    ...
    public string GetInstanceId()
    {
        ...
        if (wasSuccessful == 0) return instanceIdBuffer.ToString();
        _host?.Log("Failed to fetch device instance ID!", LogSeverity.Fatal);
        return string.Empty;
    }
    ...
    public bool UninstallDevice()
    {
        ...
        if (wasRemoved != 0)
            _host?.Log("Failed to remove device! Cannot uninstall!", LogSeverity.Fatal);

        var result = CM_Uninstall_DevNode(_deviceHandle, 0);
        if (result != 0)
            _host?.Log("Failed to uninstall device!", LogSeverity.Fatal);
    ...
    private void EnumerateChildren()
    {
        ...
        do
        {
            var childDevice = new DeviceNode(ptrChild, this, _host);
    ...
    [DllImport("cfgmgr32.dll")]
    private static extern int CM_Get_DevNode_Registry_Property_Ex(IntPtr deviceHandle, int property, IntPtr regDataType, IntPtr outBuffer, ref uint size,
        int flags, IntPtr machineHandle);
    ...
    private static extern int CM_Query_And_Remove_SubTree_Ex(IntPtr deviceInstance, out int vetoType, StringBuilder vetoName, uint vetoNameLength, int flags,
...
[Flags]
public enum DeviceNodeStatus : ulong
{
    ...
    DeviceDisconnected =
        NeedsLocking, // The function driver for a device reported that the device is not connected.  Typically this means a wireless device is out of range.
