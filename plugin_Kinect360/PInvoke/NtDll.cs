...
public static class NtDll
{
    ...
    [DllImport("ntdll.dll")]
    private static extern uint NtQuerySystemInformation(uint SystemInformationClass, ref SYSTEM_CODEINTEGRITY_INFORMATION SystemInformation,
    ...
    public static bool IsCodeIntegrityEnabled()
    {
        ...
        var codeIntegrityState = false;

        var tries = 5;
        var len = 0;
        while ((ntStatus = NtQuerySystemInformation(SystemCodeIntegrityInformation, ref codeIntegrityInfo, handleSize, ref len)) == 0xc0000004)
        {
            // InfoLengthMismatch
            handleSize = len;
            tries--;
