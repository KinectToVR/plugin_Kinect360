...
namespace plugin_Kinect360.PInvoke;
...
public static class SetupApi
{
    ...
    private enum DI_FUNCTION : uint
    {
        ...
        [Obsolete] DIF_MOVEDEVICE = 0x0000000E
    }
    ...
    private enum DI_FLAGS : uint
    {
        ...
        [Obsolete] DI_PROPS_NOCHANGEUSAGE = 0x20000000,
        [Obsolete] DI_NOSELECTICONS = 0x40000000,
    ...
    private enum DI_FLAGSEX : uint
    {
        ...
        [Obsolete] DI_FLAGSEX_NOUIONQUERYREMOVE = 0x00001000,
    ...
    [return: MarshalAs(UnmanagedType.Bool)]
    private static extern bool SetupDiOpenDeviceInfo(IntPtr DeviceInfoSet, [Optional] [MarshalAs(UnmanagedType.LPTStr)] string DeviceInstanceId,
        [In] [Optional] IntPtr hwndParent, DIOD OpenFlags, ref SP_DEVINFO_DATA DeviceInfoData);
    ...
    [DllImport("SetupAPI.dll", SetLastError = true, CharSet = CharSet.Auto)]
    [return: MarshalAs(UnmanagedType.Bool)]
    private static extern bool SetupDiGetDeviceInstallParams(IntPtr DeviceInfoSet, in SP_DEVINFO_DATA DeviceInfoData,
        ref SP_DEVINSTALL_PARAMS DeviceInstallParams);


    [DllImport("SetupAPI.dll", SetLastError = true, CharSet = CharSet.Auto)]
    [return: MarshalAs(UnmanagedType.Bool)]
    private static extern bool SetupDiSetDeviceInstallParams(IntPtr DeviceInfoSet, in SP_DEVINFO_DATA DeviceInfoData,
    ...
    [return: MarshalAs(UnmanagedType.Bool)]
    private static extern bool SetupDiEnumDriverInfo(IntPtr DeviceInfoSet, [In] [Optional] IntPtr DeviceInfoData, SPDIT DriverType, int MemberIndex,
        ref SP_DRVINFO_DATA_V2 DriverInfoData);

    [DllImport("SetupAPI.dll", SetLastError = true, CharSet = CharSet.Unicode)]
    [return: MarshalAs(UnmanagedType.Bool)]
    private static extern bool SetupDiEnumDriverInfo(IntPtr DeviceInfoSet, [In] [Optional] SP_DEVINFO_DATA DeviceInfoData, SPDIT DriverType, int MemberIndex,
        ref SP_DRVINFO_DATA_V2 DriverInfoData);


    [DllImport("SetupAPI.dll", SetLastError = true, CharSet = CharSet.Auto)]
    [return: MarshalAs(UnmanagedType.Bool)]
    private static extern bool SetupDiGetDriverInfoDetail(IntPtr DeviceInfoSet, in SP_DEVINFO_DATA DeviceInfoData, in SP_DRVINFO_DATA_V2 DriverInfoData,
        ref SP_DRVINFO_DETAIL_DATA DriverInfoDetailData, int DriverInfoDetailDataSize, out int RequiredSize);

    [DllImport("SetupAPI.dll", SetLastError = true, CharSet = CharSet.Auto)]
    [return: MarshalAs(UnmanagedType.Bool)]
    private static extern bool SetupDiGetDriverInfoDetail(IntPtr DeviceInfoSet, in IntPtr DeviceInfoData, in IntPtr DriverInfoData,
        ref SP_DRVINFO_DETAIL_DATA DriverInfoDetailData, int DriverInfoDetailDataSize, out int RequiredSize);


    [DllImport("newdev.dll", SetLastError = true, CharSet = CharSet.Auto)]
    [return: MarshalAs(UnmanagedType.Bool)]
    private static extern bool InstallSelectedDriver(IntPtr HwndParent, IntPtr DeviceInfoSet, IntPtr Reserved, [MarshalAs(UnmanagedType.Bool)] bool Backup,
        out uint Reboot);

    [DllImport("Setupapi.dll", SetLastError = true, EntryPoint = "InstallHinfSection", CharSet = CharSet.Auto)]
    private static extern void InstallHinfSection([In] IntPtr hwnd, [In] IntPtr ModuleHandle, [In] [MarshalAs(UnmanagedType.LPWStr)] string CmdLineBuffer,
        int nCmdShow);
    ...
    public static bool AssignDriverToDeviceId(string deviceInstanceId, string infPath, IDependencyInstaller.ILocalizationHost host)
    {
        var success = false;
        ...
        var DriverInfoData = new SP_DRVINFO_DATA_V2();
        ...
        try
        {
            ...
            if (DeviceInfoSet == INVALID_HANDLE_VALUE) throw new Win32Exception(Marshal.GetLastWin32Error());
            ...
            // Select our device
            if (!SetupDiSetSelectedDevice(DeviceInfoSet, DeviceInfoData)) throw new Win32Exception(Marshal.GetLastWin32Error());

            // Fetch a SP_DRVINFO_DATA structure to install on the device
            if (!SetupDiGetDeviceInstallParams(DeviceInfoSet, DeviceInfoData, ref DeviceInstallParams)) throw new Win32Exception(Marshal.GetLastWin32Error());
            ...
            if (!SetupDiSetDeviceInstallParams(DeviceInfoSet, DeviceInfoData, DeviceInstallParams)) throw new Win32Exception(Marshal.GetLastWin32Error());

            // Build a driver information list from the specified INF, and a compatible driver list
            if (!SetupDiBuildDriverInfoList(DeviceInfoSet, DeviceInfoData, SPDIT.SPDIT_COMPATDRIVER)) throw new Win32Exception(Marshal.GetLastWin32Error());
            ...
            if (!SetupDiGetSelectedDriver(DeviceInfoSet, DeviceInfoData, ref DriverInfoData)) throw new Win32Exception(Marshal.GetLastWin32Error());

            // Install the selected driver on the selected device
            success = InstallSelectedDriver(IntPtr.Zero, DeviceInfoSet, IntPtr.Zero, true, out _);
        }
    ...
    public static bool AssignExistingDriverViaInfToDeviceId(string deviceInstanceId, string infFile, string manufacturer, string description, IDependencyInstaller.ILocalizationHost host)
    {
        var success = false;
        ...
        var DeviceInstallParams = new SP_DEVINSTALL_PARAMS();
        DeviceInstallParams.cbSize = Marshal.SizeOf(DeviceInstallParams);
        var DriverInfoDetail = new SP_DRVINFO_DETAIL_DATA();
        ...
        try
        {
            ...
            // Build a driver information list from the specified INF, and a compatible driver list
            if (!SetupDiBuildDriverInfoList(DeviceInfoSet, DeviceInfoData, SPDIT.SPDIT_CLASSDRIVER)) throw new Win32Exception(Marshal.GetLastWin32Error());

            var index = 0;
            var requiredSize = Marshal.SizeOf(DriverInfoDetail);
            bool state;
            while (state = SetupDiEnumDriverInfo(DeviceInfoSet, DeviceInfoData, SPDIT.SPDIT_CLASSDRIVER, index, ref DriverInfoData))
            {
                ...
                if (SetupDiGetDriverInfoDetail(DeviceInfoSet, DeviceInfoData, DriverInfoData, ref DriverInfoDetail, Marshal.SizeOf(DriverInfoDetail),
                        out requiredSize) || Marshal.GetLastWin32Error() == 122)
                    if (DriverInfoData.MfgName == manufacturer && // If the Manufacturer is (Generic USB Audio)
                        ...
                        DriverInfoData.Description == description)
                        // If the driver display name is USB Audio Device
            ...
            // Select the driver
            if (!SetupDiSetSelectedDriver(DeviceInfoSet, ref DeviceInfoData, ref DriverInfoData)) throw new Win32Exception(Marshal.GetLastWin32Error());
